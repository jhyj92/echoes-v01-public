...
export default function GuidePage(){
  ...
  const [stage,setStage]=useState("intro"); // intro -> choices -> follow -> ai -> end
  const [aiLine,setAI]=useState("");
  const router=useRouter();

  useEffect(()=>{
    const domain=JSON.parse(localStorage.getItem("echoes_domain")||"null");
    if(!domain){router.replace("/");return;}
  ...
  async function triggerAIReflect(){
    ...
    const prompt=`User dominant traits: ${traits}. Write one poetic line like: “In your echoes, ...”`;
    const res=await fetch("/api/extractTraits",{ // reuse route for quick proxy
      method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({userInput:prompt})
    });
    const {traits:line}=await res.json();
    setAI(line); setStage("ai");
  }
    ...
    <main style={{maxWidth:"640px",margin:"10vh auto"}} className="fade">
      ...
      {stage==="choices" && (
        <div style={{display:"flex",flexDirection:"column",gap:"12px",marginTop:"20px"}}>
          {script.choices.map((c,i)=>(
            <button key={i} className="button-poetic" onClick={()=>setStage("follow")}>{c}</button>
          ))}
        </div>
      )}
      ...
      {stage==="ai" && (
        <>
          ...
          <input style={{width:"100%",padding:"10px",background:"#0F1116",color:"#fff",border:"1px solid #444"}} />
        </>
      )}
    </main>
  );
